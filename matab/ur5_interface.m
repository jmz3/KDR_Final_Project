    ...
    % Update byL Jeremy Zhang, 11/26/2023
    ...
    classdef ur5_interface < handle
        ...
        methods
            ...
            function self = ur5_interface()
                ...
                self.current_joint_states_pub = ros2publisher(self.node_handle,'/joint_states','sensor_msgs/JointState');
                ...
                self.swtich_to_pos_ctrl_client = ros2svcclient(self.node_handle, "kdr/swtich_to_pos_ctrl", 'std_srvs/Trigger');
                self.swtich_to_vel_ctrl_client = ros2svcclient(self.node_handle, "kdr/swtich_to_vel_ctrl", 'std_srvs/Trigger');
            ...
            function publish_joint_states(self,joint_goal)
            joint_state = ros2message('sensor_msgs/JointState');
            joint_state.name{1} = 'shoulder_pan_joint';
            joint_state.name{2} = 'shoulder_lift_joint';
            joint_state.name{3} = 'elbow_joint';
            joint_state.name{4} = 'wrist_1_joint';
            joint_state.name{5} = 'wrist_2_joint';
            joint_state.name{6} = 'wrist_3_joint';
    
            joint_state.header.stamp = ros2time(self.node_handle,'now');
    
            joint_state.position = joint_goal;
            joint_state.velocity = zeros(6,1);
            joint_state.effort = zeros(6,1);
    
            send(self.current_joint_states_pub,joint_state);
            end
            ...
            function move_joints(self,joint_goal,time_interval)
                ...
                for i=1:size(joint_goal,2)
                    ...
                    % velocities,accelerations,effort have to be empty for 
                    % the ros_control to solve
                    % Cannot set is as a bunch of zeros, otherwise the
                    % robot will not move
